...
namespace BF4_Friends_Checker
{
    public partial class Friends_BF4 : Form
    {
        ...
        public Friends_BF4(Friend[] friends) {
            InitializeComponent();
        ...
        void GenerateLView(ListView lView, Friend[] data) {
            ...
            for (int i = 0; i < data.Length; i++) {
                ...
                var _lastLogin = FromUnixTime(data[i]?.userinfo?.lastLogin ?? 0);
                var _since = FromUnixTime(GetEpochSeconds(dateNow) - (data[i]?.userinfo?.lastLogin ?? 0));
