using BF1_API.Authentication.Models;
using System;
...
namespace BF4_Friends_Checker
{
    public partial class Friends : Form
    {
        ...
        void GenerateLView(ListView lView, Friend[] data) {
            if (InvokeRequired) {
            ...
            for (int i = 0; i < data.Length; i++) {
                ...
                var _updatedAt = FromUnixTime(data[i]?.user?.presence?.updatedAt ?? 0);
                var _lastLogin = FromUnixTime(data[i]?.user?.presence?.profileCommon?.userinfo?.lastLogin ?? 0);
                var _since = FromUnixTime(GetEpochSeconds(dateNow) - (data[i]?.user?.presence?.profileCommon?.userinfo?.lastLogin ?? 0));
